...
day7C = liftIO (readICPFrom "data/Day7-sample4.txt") >>= interpretC >> return ()

amp ps = adp ps .| day7C
  where adp ps = yield ps >> do { (Just x) <- await; yield x }

soa a b c d e = amp a .| amp b .| amp c .| amp d .| amp e

day7Part1 = sequence $ do
  (a:b:c:d:e:_) <- permutations [0..4]
  return . runConduit $ yield 0 .| soa a b c d e .| do { Just o <- await; return o }

-- day7Part2 = sequence $ mdo
--   (a:b:c:d:e:_) <- permutations [5..9]
--   x <- runConduit $ yield x .| soa a b c d e .| do { Just x <- await; return x }
--   return x

lsoa a b c d e = mdo
  is <- runConduit $ (yield 0 >> yieldMany is) .| soa a b c d e .| sinkList
  return is
